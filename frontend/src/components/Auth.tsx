...
import type { SignupInput } from '@rahul-pandey-17/common'
...
export function Auth({ type }: { type: 'signup' | 'signin' }) {
  ...
  async function sendRequest() {
    try {
      const res = await axios.post(`${BACKEND_URL}/auth/${ type === 'signup'? 'signup': 'signin' }`, postInputs)
      const jwt = res.data.token
      localStorage.setItem('token', jwt)
      navigate('/bulk')
    } catch(err) {
      console.error('Error:', (err instanceof Error) && err.message)
